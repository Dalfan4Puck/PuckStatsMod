...
You can refer to "C:\PuckModdingTools\PuckDecompiled310Project" for a decompiled version of the updated version, and
"C:\PuckModdingTools\PuckDecompiled202Project" for a decompiled version of the old version.

You can also find the game's assets dumped in "C:\PuckModdingTools\b202-full-dump" and "C:\PuckModdingTools\b310-full-dump"; you should only look at this if you need to.
...
## 17. Additional Migration Notes (Discovered During Modding)

These are practical issues discovered while migrating real mods that aren't obvious from the API diff alone.

### Chat System Overhaul

The chat system was completely restructured. `UIChat` no longer handles sending messages — that moved to a new `ChatManager` NetworkBehaviour.

```csharp
// 202 ❌
UIChat.Instance.Client_SendClientChatMessage(message, useTeamChat);
UIChat.Instance.AddChatMessage("hello");

// 310 ✅
// Sending: use ChatManager (NetworkBehaviourSingleton)
NetworkBehaviourSingleton<ChatManager>.Instance.Client_SendChatMessage(content, isQuickChat, isTeamChat);

// UIChat is no longer a singleton — access via UIManager:
MonoBehaviourSingleton<UIManager>.Instance.Chat
```

**UIChat internal fields renamed:**
| 202 Field | 310 Field |
|---|---|
| `container` (private) | `chat` (private) |
| `chatScrollView` (private) | `scrollView` (private) |

**Chat message submission flow changed:**

- 202: `UIChat.Client_SendClientChatMessage()` sent directly
- 310: `UIChat.SubmitMessage()` triggers `EventManager.TriggerEvent("Event_OnChatSubmitMessage", ...)` → `ChatManagerController` listens → calls `ChatManager.Client_SendChatMessage()`

**Harmony patching for chat interception:**
...
// New signature: (string content, bool isQuickChat, bool isTeamChat)
```

### PostProcessingManager: Singleton → Plain MonoBehaviour

The guide lists the rename (`PostProcessingManager` → `PostProcessing`) but doesn't mention the base class change. It's no longer a `MonoBehaviourSingleton`.

```csharp
// 202 ❌
MonoBehaviourSingleton<PostProcessingManager>.Instance

// 310 ✅ — plain MonoBehaviour, find it manually
Object.FindObjectOfType<PostProcessing>()
// Or access via the PostProcessingController component on the same GameObject
```

### ConnectionManager.Client_StartClient Changes

Beyond the `ConnectionData` expansion, the connection flow itself changed:

```csharp
// 202 ❌ — State stored on ConnectionManager instance
__instance.PendingConnection = new Connection { IpAddress = ip, Port = port, Password = pw };
__instance.LastConnection = new Connection { IpAddress = ip, Port = port, Password = pw };
__instance.IsConnecting = true;

// 310 ✅ — State stored in GlobalStateManager
GlobalStateManager.SetConnectionState(new Dictionary<string, object> {
    { "pendingConnection", connection },  // or null to clear
...
**Connection struct changed:**

```csharp
// 202 ❌
new Connection { IpAddress = ip, Port = port, Password = pw }

// 310 ✅
new Connection { EndPoint = new EndPoint(ip, port), Password = pw }
```

**Password default changed:** `string password = ""` → `string password = null`

**Encoding changed:** `Encoding.ASCII.GetBytes(json)` → `Encoding.UTF8.GetBytes(json)`

**SocketId removed from ConnectionData** — no longer needed (was `WebSocketManager.Instance.SocketId`)

### Player.OnNetworkPostSpawn Rewrite

The old `Client_PlayerSubscriptionRpc` (the massive 30-parameter RPC where clients sent their username, cosmetics, admin level, etc.) is **completely removed**. Player identity data is now set server-side during `Server_SpawnPlayer()` from backend-validated `PlayerData`.

```csharp
// 202 ❌ — Client told server who it was
player.Client_PlayerSubscriptionRpc(username, number, handedness, country,
    visorBlue, visorRed, ..., patreonLevel, adminLevel, steamId, enabledModIds);

// 310 ✅ — Server sets everything from backend data
// PlayerManagerController.Event_Server_OnApprovedClientConnected:
playerManager.Server_SpawnPlayer(clientId, gameState, customizationState,
    handedness, steamID, username, number, patreonLevel, adminLevel, isMuted, isReplay);
// Identity comes from backend PlayerData, cosmetics from ConnectionData
```

**`ProcessInitialNetworkVariableValues` is private** — mods that patch `OnNetworkPostSpawn` and need to call it must use reflection:

```csharp
AccessTools.Method(typeof(Player), "ProcessInitialNetworkVariableValues").Invoke(player, null);
```

### StateManager Decomposition

`StateManager` was split into multiple static classes:

```csharp
// 202 ❌
MonoBehaviourSingleton<StateManager>.Instance.PlayerData.steamId
MonoBehaviourSingleton<StateManager>.Instance.PlayerData.username

// 310 ✅
BackendManager.PlayerState.PlayerData.steamId    // Player identity/backend data
BackendManager.PlayerState.Key                    // Session auth key (new)
GlobalStateManager.ConnectionState.IsConnecting   // Connection state
GlobalStateManager.UIState.Phase                  // UI state
```

### WebSocketManager.Emit Unchanged but Access Changed

```csharp
// 202 ❌
MonoBehaviourSingleton<WebSocketManager>.Instance.Emit("event", data, "response");

// 310 ✅
WebSocketManager.Emit("event", data, "response");
// Same signature, just static now

// New in 310 — message listeners:
WebSocketManager.AddMessageListener("eventName", handler);
WebSocketManager.RemoveMessageListener("eventName", handler);
```

---

## 18. Puck 310 → 312 Changes

> **Decompiled sources:** `C:\PuckModdingTools\PuckDecompiled312Project` (new), `C:\PuckModdingTools\PuckDecompiled310Project` (old)

### New Files (5) — Player Statistics System

New DTO hierarchy for exposing live server/matchmaking stats to clients:

| New Class | Fields |
|---|---|
| `PlayerStatistics` | `PlayerManagerStatistics playerManager`, `ServerManagerStatistics serverManager`, `MatchmakingManagerStatistics matchmakingManager` |
| `PlayerManagerStatistics` | `int players` |
| `ServerManagerStatistics` | `int publicServers` |
| `MatchmakingManagerStatistics` | `PoolStatistics[] pools` |
| `PoolStatistics` | `string id`, `double? averageMatchingDuration`, `int groupPlayers` |

`PlayerState` now has a `PlayerStatistics` field — accessible via `BackendManager.PlayerState`.

### Breaking: MaxMatchmakingRtt → MaxMatchmakingPing

Renamed throughout settings:

```csharp
// 310 ❌
SettingsManager.MaxMatchmakingRtt
"Event_OnMaxMatchmakingRttChanged"
// SaveManager key: "maxMatchmakingRtt"

// 312 ✅
SettingsManager.MaxMatchmakingPing
"Event_OnMaxMatchmakingPingChanged"
// SaveManager key: "maxMatchmakingPing"
```

### Breaking: MeshRendererTexturer.Enable()/Disable() Removed

```csharp
// 310 ❌
meshRendererTexturer.Enable();
meshRendererTexturer.Disable();

// 312 ✅ — Use the GameObject directly
meshRendererTexturer.gameObject.SetActive(true);
meshRendererTexturer.gameObject.SetActive(false);
```

`PlayerHead.cs` now uses `FlagGameObject.SetActive()` instead of `FlagMeshRendererTexturer.Enable()/Disable()`.

### Breaking: Spectator.updateRate Removed

The `updateRate` field (was `15f`) is removed from `Spectator`. Spectator update rate is no longer configurable per-instance.

### Breaking: UIManager Mouse → Pointer Events
...
PointerEnterEvent
PointerDownEvent
// Also new: PointerCaptureOutEvent for click sounds
```

Button hover/click sounds now also check `visualElement.enabledInHierarchy` — disabled buttons no longer play sounds.

### New: Match Join Timeout System

New constant and fields for tracking match join countdown:

```csharp
Constants.MATCH_JOIN_TIMEOUT  // 60f seconds

// MatchData gains:
matchData.startedAt  // double? — when match was found

// PlayerMatchData gains:
playerMatchData.JoinTimeoutRemainingSeconds  // computed from startedAt + MATCH_JOIN_TIMEOUT
```

New tween-based ticker methods on `BackendManager`:

```csharp
BackendManager.StartMatchingTicker();
BackendManager.StopMatchingTicker();
BackendManager.StartMatchJoinTimeoutTicker();
BackendManager.StopMatchJoinTimeoutTicker();
```

New events:

- `Event_OnMatchingTickerStarted`, `Event_OnMatchingTickerTick`, `Event_OnMatchingTickerStopped`
- `Event_OnMatchJoinTimeoutTickerStarted`, `Event_OnMatchJoinTimeoutTickerTick`, `Event_OnMatchJoinTimeoutTickerStopped`
- `Event_OnPlayerStatisticsChanged`

### New: SteamIntegrationManager.IsFriend()

```csharp
// 312 ✅
SteamIntegrationManager.IsFriend(string steamId)  // returns bool
```

`OnPersonaStateChange` now filters events — only fires `Event_OnPersonaStateChange` for name, status, avatar, relationship, or nickname changes (was firing for all persona state changes).

### SaveManager: Debounced Saves

`PlayerPrefs.Save()` is now debounced via DOTween instead of being called immediately. Multiple rapid saves are coalesced into one.

### ModManager/InputManager: PlayerPrefs → SaveManager Migration

```csharp
// 310 ❌
PlayerPrefs.GetString("modsState")
PlayerPrefs.SetString("modsState", value)
PlayerPrefs.GetString("keyBinds")
PlayerPrefs.SetString("keyBinds", value)

// 312 ✅
SaveManager.GetString("modsState")
SaveManager.SetString("modsState", value)
SaveManager.GetString("keyBinds")
SaveManager.SetString("keyBinds", value)
```

If your mod was reading/writing these PlayerPrefs keys directly, switch to `SaveManager`.

### InputManager: Reset-to-Default Gated Behind Popup

`InputManagerController` now listens for `Event_OnPopupClickOk` (instead of `Event_OnSettingsResetToDefault`) and checks `message["name"] == "settingsResetToDefault"` before resetting. Key bind reset only happens when the user confirms the popup.

### LogManager: Thread Safety + UtcNow

- Log writes are now wrapped in a `streamWriterLock` for thread safety
- Uses a custom `LogHandler` class wrapping Unity's `ILogHandler`
- Log timestamps changed from `DateTime.Now` to `DateTime.UtcNow`
- `UIPopupManagerController` also changed from `DateTime.Now.ToLocalTime()` to `DateTime.UtcNow`

### LifecycleManager: Earlier Initialization

`LogManager.Initialize()` and `Application.quitting` registration moved from `Awake()` to a `[RuntimeInitializeOnLoadMethod(SubsystemRegistration)]` method — this runs earlier in the Unity lifecycle, before any MonoBehaviour `Awake()` calls.

### ConnectionManagerController: Net Stats Visibility

Network stats monitor now requires `Debug AND IsConnected` (was just `Debug`). New `UpdateRnsmVisibility()` method called on `Event_OnConnectionStateChanged`.

### UIPlay: New Methods for Queue Stats

```csharp
// 312 ✅ — Play buttons can be disabled with descriptions
uiPlay.SetThreeVsThreeButtonEnabled(bool enabled);
uiPlay.SetThreeVsThreeButtonDescription(string desc);
uiPlay.SetFiveVsFiveButtonEnabled(bool enabled);
uiPlay.SetFiveVsFiveButtonDescription(string desc);
// New UI elements: statistics, playersLabel
```

### Bug Fixes in 312

- **Server deploy toast**: Logic was inverted — successful deploys showed nothing and failures showed the error. Fixed to show "Server deployed!" on success.
- **SSL certificate validation**: Boolean check for cert validation was inverted. Fixed which mode gets strict vs bypassed validation.
- **WebSocket reconnect**: `WebSocketManager.Disconnect()` moved to controller; reconnect on error now properly awaited.

### No Changes to Core Gameplay / Mod API

The following had **no meaningful logic changes** between 310 and 312 (decompiler noise only):

`Player.cs`, `PlayerController.cs`, `PlayerBody.cs`, `PlayerBodyController.cs`, `ServerManager.cs`, `GameManager.cs`, `ConnectionManager.cs`, `IPuckMod.cs`, `Mod.cs`, `ModManagerController.cs`, `Puck.cs`, `PuckController.cs`, `Stick.cs`, `StickController.cs`, `PlayerManager.cs`, `PlayerManagerController.cs`

The `IPuckMod` interface remains unchanged.
